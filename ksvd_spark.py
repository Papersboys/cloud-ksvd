...
t_d = 2 # iterations of K-SVD
t_p =  3 # iterations of distributed power
...
def parseVector(line):
    ...
    arrSz = arr.size
    arr_mod = np.reshape(arr, (arrSz,1)) #can it be removed?
...
if __name__ == "__main__":
    startTime = time.time()
    conf = (SparkConf().setAppName("KSVD").set("spark.eventLog.dir","file:/home/labuser/Documents/spark/spark-history" ).set("spark.history.fs.logDirectory","file:/home/labuser/Documents/spark/spark-history")
        .set("spark.eventLog.enabled","true").set("spark.executor.memory", "6g").set("spark.storage.memoryFraction", "0.4"))
    ...
    dataW = sc.textFile(sys.argv[3]).flatMap(parseStochMat).reduceByKey(lambda arrA, arrB: np.concatenate((arrA, arrB), axis=1)).cache() 

    #used repeatedly in distributed power iteration
    #dataW is the complete doubly stochastic matrix and is avialable at every site (key-Site, val-matrix).
    #Note, instead of storing the matrix, we store the W^(t_c) as that is what is required for compuattion and 
    #we do not want to do matrix multiplication evrytime in the lopp
    #
    ...
    for d in range(t_d):
        ...
        for k in range(K):
            ...
            for p in range(t_p):
                #everything done locally up to now
    ...
    for (site, (output)) in dataOut:
        ...
        D_final = output[0]
        X_final = output[1]
        x_file = 'output/Site_' + str(site) + '_X.txt'
        d_file = 'output/Site_' + str(site) + '_D.txt'
